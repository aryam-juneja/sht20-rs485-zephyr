...
int main(void)
{
    ...
    uint8_t query_read_temp_hum[8] = {0x01, 0x04, 0x00, 0x01, 0x00, 0x02, 0x20, 0x0B};

    uint8_t rx_buf[9];   // Buffer to store received data

    while (1) {
        send_uart_command(query_read_temp_hum, sizeof(query_read_temp_hum));

        int ret = receive_uart_data(rx_buf, sizeof(rx_buf), K_MSEC(100));
        if (ret == 0) {
            // Check for the correct frame header
            if (rx_buf[0] == 0x01 && rx_buf[1] == 0x04) {
                // Print received data bytes for debugging
                printk("\rReceived Data:");
                for (int i = 0; i < sizeof(rx_buf); i++) {
                    printk(" %02X", rx_buf[i]);
                }
                printk("\n");

                uint16_t temp = ((rx_buf[3] << 8) | rx_buf[4]);
                uint16_t humi = ((rx_buf[5] << 8) | rx_buf[6]);
